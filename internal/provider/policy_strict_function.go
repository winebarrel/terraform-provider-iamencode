...
func (r PolicyStrictFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Like `policy`, but also validates the policy against the live " +
			...
			"process, so a single plan makes at most one HTTP call per referenced service. Transient fetch failures " +
			"(HTTP 429/5xx and network errors) are retried up to 3 times with exponential backoff. If the reference " +
